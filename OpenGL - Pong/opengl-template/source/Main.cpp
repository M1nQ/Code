...
static void loop(SDL_Window* window)
{
	...
	// Variables for counting deltaTime
	...
	double previousTotalTimeElapsed = 0.0;
	...
	while (!exit)
	{
		...
		double totalTimeElapsed = (currentCount.QuadPart - startCount.QuadPart) / (double)frequency.QuadPart;
		double deltaTime = totalTimeElapsed - previousTotalTimeElapsed;
